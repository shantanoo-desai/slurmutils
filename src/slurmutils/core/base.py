...
if sys.version_info >= (3, 14):
    ...
    # In Python 3.14+, PEP 649 introduces lazy evaluation of annotations, meaning
    # `__annotations__` is no longer eagerly populated as a dictionary in the class
    # __dict__/metaclass `namespace` argument. Instead, we use
    # `annotationlib.get_annotations()` (with a fallback shim for < 3.14) to
    # safely retrieve them across all Python versions.
    class Format:
        VALUE = 1
